...
class Common
{
  ...
  public function  mailErrorCheck($dataArr)
  {
    $this->dataArr = $dataArr;
    // クラス内のメソッドを読み込む
    $this->createErrorMessage();
  ...
  public function kintaiCheck($dataArr){
    $this->dataArr = $dataArr;
    // クラス内のメソッドを読み込む
    $this->createErrorMessage();
    $this->mailCheck();
  ...
  private function idCheck()
  {
    ...
    {
        $this->errArr['user_id'] = 'ユーザ名を入力してください';
    }
  }
  ...
  private function workCheck(){
    if ($this->dataArr['start'] === []) {
      $this->errArr['start'] = '始業時間を入力してください。';
    }elseif($this->dataArr['end'] === []){
      $this->err['end'] = '終業時間を入力してください';
    }
  }
